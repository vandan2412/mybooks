...
public class sale2 extends AppCompatActivity {
    ...
    // Watcher references (clasV-level so we can remove/re-add safely)
    ...
    private void rollbackInvoiceIfNeeded() {
        ...
        String inv = etInvoice.getText().toString();   // example: V-5
        if (inv == null || !inv.startsWith("V-")) return;
        ...
        DocumentReference invRef = db.collection("invno").document("V-");
    ...
    private void setupDateAndInvoice() {
        ...
        etDate.setOnClickListener(v -> {
            ...
            new DatePickerDialog(sale2.this, (view, year, month, day) ->
        ...
        etSearchItem.setOnItemClickListener((parent, view, position, id) -> {
            ...
            for (Map<String, Object> i : itemList) {
                if (i.get("itemName").toString().equals(itemName)) {
                    ...
                    String wholesaleRate = i.get("wholesaleRate") != null ? i.get("wholesaleRate").toString() : "";
                    etPrice.setText(wholesaleRate);
...
// --- NEW CODE: Correct party balance update ---
                                    ...
                                    invRef.get().addOnSuccessListener(doc -> {
                                        if (doc.exists() && doc.contains("number")) {
                                            long savedNum = doc.getLong("number");
                                            etInvoice.setText("V-"+savedNum);
                                            loadSaleByInvoice("V-"+savedNum);
                                    ...
                                    Intent intent = new Intent(sale2.this, PrintPreviewActivit.class);
                       ...
                       int insertionPoint = (insertion < 0) ? (-(insertion + 1)) : insertion;
                        ...
                        if (isNext) {
                            ...
                            if (insertionPoint >= nums.size()) {
                                ...
                                String newInvoiceNo = "V-" + newInvNumber;
                    ...
                    if (newIndex >= nums.size()) {
                        ...
                        invRef.get().addOnSuccessListener(doc -> {
                            if (doc.exists() && doc.contains("number")) {
                                ...
                                String newInvoiceNo = "V-" + (savedNum-1);
                                etInvoice.setText(newInvoiceNo);
                                loadSaleByInvoice(newInvoiceNo);
                            }
                        });

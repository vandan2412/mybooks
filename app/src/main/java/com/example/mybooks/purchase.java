...
public class purchase extends AppCompatActivity {
    ...
    Button btnAddItem, btnSave, btnCancel, btnNext, btnPrev, btnDeletePurchase;
    ListView listViewItems;
    TextView tvGrandTotal, tvFinalTotal;
    EditText etDiscountPercent, etTaxPercent;

    FirebaseFirestore db;
    ArrayList<Map<String, Object>> PurchaseItems = new ArrayList<>();
    ItemAdapter adapter;
    ...
    // Watcher references (clasP-level so we can remove/re-add safely)
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btnDeletePurchase = findViewById(R.id.btnDeletePurchase);
        ...
        btnSave.setOnClickListener(v -> saveFullPurchase());
        ...
        btnDeletePurchase.setOnClickListener(v -> confirmAndDeletePurchase());
        ...
        listViewItems.setOnItemClickListener((parent, view, position, id) -> {
            new AlertDialog.Builder(this)
                    ...
                    .setItems(new String[]{"Modify", "Delete"}, (dialog, which) -> {
                        if (which == 0)
                            showAddItemDialog(PurchaseItems.get(position), position);
                        else {
    ...
    protected void onResume() {
        ...
        if (!currentInvoice.isEmpty()) {
            loadPurchaseByInvoice(currentInvoice);
        }
    ...
    private void rollbackInvoiceIfNeeded() {
        ...
        String inv = etInvoice.getText().toString();   // example: P-5
        if (inv == null || !inv.startsWith("P-")) return;
        ...
        DocumentReference invRef = db.collection("invno").document("P-");
    ...
    private void setupDateAndInvoice() {
        ...
        etDate.setOnClickListener(v -> {
            ...
            new DatePickerDialog(purchase.this, (view, year, month, day) ->
        ...
        invRef.get().addOnSuccessListener(doc -> {
            ...
            long next = last + 1;
            String nextInvoice = "P-" + last;
            etInvoice.setText(nextInvoice);

            loadPurchaseByInvoice(nextInvoice);
            ...
            String defaultInvoice = "P-1";
            etInvoice.setText(defaultInvoice);
            loadPurchaseByInvoice(defaultInvoice);
    ...
    private void showAddItemDialog(Map<String, Object> existing, int editPosition) {
        ...
        etSearchItem.setOnItemClickListener((parent, view, position, id) -> {
            ...
            for (Map<String, Object> i : itemList) {
                if (i.get("itemName").toString().equals(itemName)) {
                    ...
                    String purchasePrice = i.get("purchasePrice") != null ? i.get("purchasePrice").toString() : "";
                    etPrice.setText(purchasePrice);
        ...
        btnAdd.setOnClickListener(v -> {
            ...
            if (existing == null) {
                ...
                PurchaseItems.set(editPosition, item);
            }
    ...
    private void recalculateTotal() {
        try {
            ...
            // --- 3. Read Percentages and Amounts (allow negative for returns) ---
        ...
        db.collection("Party")
                ...
                .addOnCompleteListener(task -> {
                    ...
                    if (!task.isSuccessful() || task.getResult().isEmpty()) {
                        Toast.makeText(this, "Party does not exist. Cannot save Purchase.", Toast.LENGTH_SHORT).show();
                        return;
                    }

                    db.collection("Purchases").document(invoiceNo).get().addOnSuccessListener(existing -> {
                        ...
                        PurchaseData.put("invoiceNo", invoiceNo);
                        PurchaseData.put("date", date);
                        PurchaseData.put("party", partyText);
                        PurchaseData.put("items", PurchaseItems);
                        PurchaseData.put("discount", discount);
                        PurchaseData.put("tax", tax);
                        PurchaseData.put("grandTotal", finalCalculatedTotal);
                        PurchaseData.put("remarks", etRemarks.getText().toString().trim());

                        String finalOldParty = oldParty;
                        double finalOldTotal = oldTotal;
                        db.collection("Purchases").document(invoiceNo).set(PurchaseData)
...
// --- NEW CODE: Correct party balance update ---
                                    ...
                                    updateItemStocks(oldItems, new ArrayList<>(PurchaseItems));
                                    ...
                                    if (finalCalculatedTotal < 0) {
                                        journalEntry.put("to", "Purchases Account");
                                        journalEntry.put("from", partyText);
                                        journalEntry.put("amount", -finalCalculatedTotal);
                                        journalEntry.put("remarks", "Purchase Return for " + invoiceNo);
                                        journalEntry.put("type", "Purchase Return");
                                    } else {
                                        journalEntry.put("to", partyText);
                                        journalEntry.put("from", "Purchases Account");
                                        journalEntry.put("amount", finalCalculatedTotal);
                                        journalEntry.put("remarks", "Purchase Entry for " + invoiceNo);
                                        journalEntry.put("type", "Purchase");
                                    }
                                    ...
                                    db.collection("Journal").document(invoiceNo).set(journalEntry)
                                            .addOnSuccessListener(v -> {
                                                ...
                                                if (discount != 0) {
                                                    ...
                                                    if (discount < 0) {
                                                        discountEntry.put("to", "Discount Received");
                                                        discountEntry.put("from", "Purchases Account");
                                                        ...
                                                        discountEntry.put("to", "Purchases Account");
                                    ...
                                    invRef.get().addOnSuccessListener(doc -> {
                                        if (doc.exists() && doc.contains("number")) {
                                            long savedNum = doc.getLong("number");
                                            etInvoice.setText("P-"+savedNum);
                                            loadPurchaseByInvoice("P-"+savedNum);
                                    ...
                                    bill.items = new ArrayList<>(PurchaseItems);
                                    ...
                                    Intent intent = new Intent(purchase.this, PrintPreviewActivit.class);
    ...
    private void confirmAndDeletePurchase() {
        String invoiceNo = etInvoice.getText().toString();
        new AlertDialog.Builder(this)
                .setTitle("Confirm Deletion")
                .setMessage("Are you sure you want to delete Purchase Invoice: " + invoiceNo + "? This action cannot be undone, and stock/balance will be adjusted.")
                .setPositiveButton("Yes, Delete", (dialog, which) -> deletePurchase(invoiceNo))
                .setNegativeButton("Cancel", null)
                .show();
    }
    private void deletePurchase(String invoiceNo) {
        db.collection("Purchases").document(invoiceNo).get().addOnSuccessListener(d -> {
            if (!d.exists()) {
            ...
            if (oldData != null) {
                ...
                Toast.makeText(this, "Error reading old Purchase data.", Toast.LENGTH_SHORT).show();
                return;
            }
            ...
            db.collection("Purchases").document(invoiceNo).delete()
                    .addOnSuccessListener(aVoid -> {
                        ...
                        Toast.makeText(this, "Purchase invoice " + invoiceNo + " successfully deleted.", Toast.LENGTH_LONG).show();
                        navigateInvoice(true);
                    })
                    .addOnFailureListener(e ->
                            Toast.makeText(this, "Error deleting Purchase: " + e.getMessage(), Toast.LENGTH_SHORT).show());
        });
    }
    ...
    private void convertToItemUnit(String itemUnit, String billUnit, double qty,
                                   sale.OnQuantityConverted callback) {
    ...
    private void navigateInvoice(boolean isNext) {
        ...
        // Fetch all Purchases documents ONCE
        db.collection("Purchases")
                ...
                .addOnSuccessListener(querySnapshot -> {
                    ...
                    if (nums.isEmpty()) {
                        ...
                        Log.d("NavigateInv", "No numeric invoices parsed from Purchases.");
                        return;
                    }
                    ...
                    if (index == -1) {
                        ...
                        if (isNext) {
                            ...
                            if (insertionPoint >= nums.size()) {
                                ...
                                String newInvoiceNo = "P-" + newInvNumber;
                                etInvoice.setText(newInvoiceNo);
                                loadPurchaseByInvoice(newInvoiceNo);
                                return;
                            }
                        } else {
                    ...
                    if (newIndex >= nums.size()) {
                        ...
                        invRef.get().addOnSuccessListener(doc -> {
                            if (doc.exists() && doc.contains("number")) {
                                ...
                                String newInvoiceNo = "P-" + (savedNum-1);
                                etInvoice.setText(newInvoiceNo);
                                loadPurchaseByInvoice(newInvoiceNo);
                            }
                        });
                ...
                .addOnFailureListener(e -> {
                    ...
                    Log.e("NavigateInv", "Failed to read Purchases collection", e);
                });
    }

    private void loadPurchaseByInvoice(String invoiceNo) {
        db.collection("Purchases").document(invoiceNo).get()
                .addOnSuccessListener(d -> {
                    ...
                    if (d.exists()) {
                        btnDeletePurchase.setVisibility(View.VISIBLE);
                        btnSave.setText("Update");

                        Map<String, Object> PurchaseData = d.getData();
                        if (PurchaseData != null && PurchaseData.containsKey("items")) {
                            ArrayList<Map<String, Object>> items =
                                    (ArrayList<Map<String, Object>>) PurchaseData.get("items");
                        ...
                        db.collection("Journal").document(invoiceNo).get()
                                .addOnSuccessListener(journalDoc -> {
                                    if (journalDoc.exists()) {
                                        ...
                                        if ("Purchases Account".equals(from)) {
                        ...
                        // New invoice
                        btnDeletePurchase.setVisibility(View.GONE);
    ...
    private void sortItemsBySNo() {
        Collections.sort(PurchaseItems, (a, b) -> {
            int sno1 = 0;
            int sno2 = 0;
            // Defensive parsing
    ...
    class ItemAdapter extends BaseAdapter {
        ...
        public View getView(int i, View v, ViewGroup parent) {
            ...
            Map<String, Object> item = PurchaseItems.get(i);
